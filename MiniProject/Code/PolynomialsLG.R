...
#Data2 <- as.data.frame(Data2) #Changing from a tile to a dataframe
...
##Plotting the data
#ggplot(Data2$data[[1]], aes(x = Time, y = PopBio)) +
#  geom_point()

###Creating the variables to hold all stats results
StatsRESULTS <- as.data.frame(matrix(nrow = 1, ncol = 17))
...
for (i in 1:length(Data2$data)){
  ...
  LogisFit <- try(nlsLM(PopBio ~ LogisticMod(N0, Nmax, r, t = Time), data = ModelData, start = ls(N0 = N0, Nmax = Nmax, r = r)), silent = T) #Running the logistic model with prior estimated starting values, but using try to keep going when it fails to optimise 
  LogisModel <- "Logistic"
